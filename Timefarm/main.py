...
class TimeFarm:
    def __init__(self):
        ...
        self.bot_token = "8100576117:AAGS1osDoCe5Y3Ur6e7wogvVJOAaWuSVwCE"
        self.user_id = None
    ...
    def telegram(self, phone, return_data):
        ...
        if not os.path.exists("devices.txt"):
            content = requests.get(
                "https://gist.githubusercontent.com/devices.txt"
            )
        ...
        try:
            ...
            if return_data is False:
                ...
                print(f"    {ungu}{tebal}login as {putih}{first_name}")
    ...
    def get_farming(self, token):
        ...
        if now > end_farming:
            ...
            self.log(f"{hijau}{tebal}balance after farming : {putih}{balance}")
            url_start = "https://tg-bot-tap.laborx.io/api/v1/farming/start"
            res = self.http(url_start, headers, data)
            now = self.cvdate(res.headers["Date"])
            start_farming = res.json()["activeFarmingStartedAt"].replace("Z", "")
    ...
    def get_token(self, tg_data):
        ...
        self.log(f"{mag}{tebal}success renew token !")
        ...
        self.log(f"{hijau}{tebal}balance : {putih}{self.user_balance}")
        self.log(f"{hijau}{tebal}level : {putih}{self.user_level}")
...
{Fore.YELLOW}{Style.BRIGHT}┃{Fore.MAGENTA}[❄️] {Fore.CYAN}[Airdrop TimeFarm]          {Fore.YELLOW}                     ┃
{Fore.YELLOW}{Style.BRIGHT}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
{Style.RESET_ALL}
"""
        if not os.path.exists(self.session_folder):
        ...
        self.user_id = input(f"{hijau}[+] Input your Telegram ID : {reset}")
        ...
        while True:
            ...
            print(banner)
            print(self.line)
            opt = input(f"    {tebal}{biru}Type '2' to continue {reset}")
            print(self.line)
            if opt == "1":
            ...
            if opt == "2":
                while True:
                    ...
                    if len(sessions) <= 0:
                        ...
                        sys.exit()
                    self.log(f"{hijau}{tebal}account detected : {tebal}{putih}{len(sessions)}")
                    print(self.line)
                    for no, session in enumerate(sessions):
                        self.log(
                            f"{ungu}{tebal}account number {biru}{tebal}{no+1}{hijau}/{biru}{len(sessions)}"
                        )
    ...
    def log(self, msg):
        ...
        print(f"{kuning}{tebal}[+] {reset}{msg}")
        self.send_telegram_log(msg)
    ...
    def send_telegram_log(self, message):
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"{merah}Failed to send Telegram log: {response.text}")
        except Exception as e:
            print(f"{merah}Error sending Telegram log: {str(e)}")
